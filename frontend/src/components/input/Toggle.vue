<template>
  <div :class="classList.container" :tabindex="disabled ? undefined : 0" :aria-checked="checked"
    :aria-describedby="describedby" :aria-labelledby="labelledby" role="switch" v-bind="aria"
    @keypress.space.prevent="handleSpace">
    <input v-show="false" type="checkbox" :id="id" :name="name" :value="trueValue" :checked="checked"
      :disabled="disabled" />
    <div :class="classList.toggle" @click="handleClick">
      <span :class="classList.handle"></span>
      <slot name="label" :checked="checked" :classList="classList">
        <span :class="classList.label" v-html="label"></span>
      </slot>
